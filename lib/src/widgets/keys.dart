import 'package:flutter/material.dart';

const kPropertiesIcon = Key('properties_icon');
const kActionsIcon = Key('actions_icon');
const kActionsCloseIcon = Key('actions_close_icon');
const kStoriesIcon = Key('stories_icon');
const kStoriesCloseIcon = Key('stories_close_icon');
const kStoriesFilterField = Key('stories_filter_field');
const kDevicePreviewIcon = Key('device_preview_icon');
const kDevicePreviewCloseIcon = Key('device_preview_close_icon');
const kCustomDeviceToggle = Key('device_custom_toggle');
const kRotateIcon = Key('rotate_icon');
const kHideFrameIcon = Key('hide_frame_icon');
const kStoryPinIcon = Key('story_pin_icon');
